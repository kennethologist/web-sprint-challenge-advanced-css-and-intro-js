...
function getArtistByIndex(arr, arrIndex) {
  /*Your Code Here*/
  return `the artist at index ${arrIndex} is ${arr[arrIndex].name}`;
}
...
function get20s(arr) {
  /*Your Code Here*/
  const artistsFrom20th = [];

  for (let index = 0; index < arr.length; index++) {
    const element = arr[index].years.split('-');
    if (Number(element[0]) >= 1900 && Number(element[1]) <= 2000) {
      artistsFrom20th.push(arr[index].name);
    }
  }

  return artistsFrom20th;
}
...
function removeArtist(arr, indexToRemove) {
  /*Your Code Here*/
...
function addArtist(arr) {
  ...
  arr.push( { 
    ...
    bio: "I'm 36 years old. 6' 2\" and I love coding."
  }  );
...
function lotsOfArt(arr) {
  /*Your Code Here*/
  const paintedMoreThan100 = [];

  for (let index = 0; index < arr.length; index++) {
    if (arr[index].paintings > 100) {
      paintedMoreThan100.push(arr[index].name);
    }
    
  }

  return paintedMoreThan100;
}
